...
data class PendingEntry(
    ...
    /**
     * JSON-serialized list of pipeline-variable names the user has marked
     * for removal in the Inbox edit sheet. The worker drops these keys from
     * the pipeline input AND strips the matching elements from the
     * archive's ComicInfo.xml at processing time. NULL means "no removals".
     */
    val metadataRemovalsJson: String? = null,
    /**
...
interface PendingDao {
    ...
    @Query("UPDATE pending SET metadataOverridesJson = :overridesJson, metadataRemovalsJson = :removalsJson WHERE id = :id")
    suspend fun setMetadataEditsJson(id: String, overridesJson: String?, removalsJson: String?)
...
// `exportSchema = true` writes a JSON snapshot of the schema to
// `app/schemas/com.mangako.app.data.db.HistoryDatabase/<version>.json`
// each time the schema changes. Those files get checked into the repo
// — Room reads them when generating tests + Migration helpers, and
// they're the only way to write a real Migration class without
// hand-typing the previous DDL. Required before turning off
// fallbackToDestructiveMigration.
@Database(entities = [HistoryEntry::class, PendingEntry::class], version = 7, exportSchema = true)

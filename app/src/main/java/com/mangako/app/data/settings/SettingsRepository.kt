...
class SettingsRepository @Inject constructor(
    ...
    data class Settings(
        ...
        /**
         * One-shot guard for the "preload LANraragi defaults" migration. The
         * pipeline DataStore's defaultValue covers truly-fresh installs (no
         * pipeline.json exists), but users who tested an earlier build often
         * end up with an empty pipeline.json on disk that the default never
         * reaches. The first time the app starts and finds this flag false,
         * it runs the migration: if the pipeline has zero rules, load the
         * standard template; either way set the flag so a subsequent
         * intentional Clear stays cleared.
         */
        val pipelineInitialized: Boolean = false,
    )
    ...
    suspend fun update(block: (Settings) -> Settings) {
        context.dataStore.edit { prefs ->
            ...
            prefs[KEY_PIPELINE_INIT] = next.pipelineInitialized
        }
    }
    ...
    private fun Preferences.toSettings(): Settings {
        return Settings(
            ...
            pipelineInitialized = this[KEY_PIPELINE_INIT] ?: false,
        )
    }
    ...
    companion object {
        ...
        private val KEY_PIPELINE_INIT = booleanPreferencesKey("pipeline_initialized")
    }
}

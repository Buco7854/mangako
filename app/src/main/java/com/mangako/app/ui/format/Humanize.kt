 ...
 *
 * The verdict is rule-type-aware: previously every non-filename-mutating
 * rule (ExtractXmlMetadata, ExtractRegex, SetVariable on a variable,
 * WriteComicInfo) showed "No change — the filename already looked
 * right", which is misleading because those rules aren't trying to edit
 * the filename in the first place. Now each rule type gets a verdict
...
fun AuditStep.humanize(): HumanizedStep {
    ...
    return HumanizedStep(
        title = title,
        verdict = if (skipped) "Skipped — ${humanizeSkipReason(skippedReason)}"
        else verdictFor(this),
    )
}
...
/**
 * Per-rule-type verdict text. Falls back to the executor-supplied note
 * when present, since most variable / ComicInfo rules already produce
 * a meaningful one-liner there.
 */
private fun verdictFor(step: AuditStep): String = when (step.ruleType) {
    "ExtractXmlMetadata" -> step.note?.takeIf { it.isNotBlank() }
        ?: "Loaded variables from ComicInfo.xml."

    "ExtractRegex" -> step.note?.takeIf { it.isNotBlank() }
        ?: "Pulled a value out of the source variable."

    "SetVariable" ->
        if (step.changed) step.note?.takeIf { it.isNotBlank() } ?: "Renamed."
        else step.note?.takeIf { it.isNotBlank() } ?: "Set the variable."

    "WriteComicInfo" -> step.note?.takeIf { it.isNotBlank() }
        ?: "Queued ComicInfo writes for after the pipeline."

    "Group" -> step.note?.takeIf { it.isNotBlank() }
        ?: "Ran the nested rules."

    "ConditionalFormat" -> step.note?.takeIf { it.isNotBlank() }
        ?: "Took a branch."

    "RegexReplace", "StringAppend", "StringPrepend",
    "TagRelocator", "CleanWhitespace",
    -> if (step.changed) "Renamed." else "No change to the filename."

    else -> if (step.changed) "Renamed." else "Nothing to do."
}

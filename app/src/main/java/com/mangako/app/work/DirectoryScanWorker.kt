...
class DirectoryScanWorker @AssistedInject constructor(
    ...
    private suspend fun handleApproval(
        ...
        // Peek the ComicInfo so the Inbox card can show pipeline-
        // simulated %title% straight away. We DO NOT pre-extract the
        // thumbnail here any more — [ThumbnailService] generates one
        // on-demand the first time the Inbox card composes, and caches
        // it for subsequent loads. Skipping the eager extract keeps the
        // scan worker fast on directories that pile up dozens of new
        // .cbz files at once.
        val metadata = runCatching {
            cbzProcessor.extractDetectionFromUri(applicationContext, child.uri).metadata
        }.getOrDefault(emptyMap())

        val result = pendingRepo.addIfAbsent(
            uri = child.uri.toString(),
            name = name,
        ...
        val row = result.file
        // Only fire the "new file detected" notification when this scan
        // actually inserted a row. Periodic re-scans for the same file
        // see wasInserted=false and skip — the user already got the
        // notification when the file first appeared.
        if (result.wasInserted && notify) {
        ...
        // Refresh metadata on the existing row when we now have something
        // and the row didn't (file was caught mid-write last scan, complete
        // now). Thumbnails are no longer stored on the row — the service
        // owns that lifecycle.
